...
function LightboxModal({
  ...
  const likeCount = (likes[currentItem.id] || 0)
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-3 sm:top-4 left-3 sm:left-4 right-3 sm:right-4 flex justify-between items-center z-50 pointer-events-none">
        <div className="pointer-events-auto px-3 sm:px-4 py-1.5 rounded-full glass text-[11px] sm:text-xs text-[#6b5d4f] flex items-center gap-1.5 sm:gap-2 border border-[#2d2418]/10 shadow-lg">
          <SparkleIcon className="w-3.5 h-3.5 text-bloom-neon animate-pulse shrink-0" />
          <span>
            Bloom <strong className="text-[#2d2418]">{currentIndex + 1}</strong> of{' '}
            <strong className="text-[#2d2418]">{items.length}</strong>
          </span>
        </div>
        <button
          onClick={onClose}
          className="pointer-events-auto p-2 sm:p-2.5 rounded-full glass text-[#6b5d4f] hover:text-white hover:bg-bloom-rose/30 transition-all border border-[#2d2418]/10 hover:border-bloom-rose/50 active:scale-95 min-h-[40px] min-w-[40px] flex items-center justify-center"
      ...
      <button
        onClick={handleNext}
        className="absolute right-2 sm:right-6 top-1/2 -translate-y-1/2 z-50 p-2.5 sm:p-3.5 rounded-full glass text-white/90 hover:text-white hover:bg-bloom-rose/40 hover:scale-110 shadow-[0_0_15px_rgba(255,64,129,0.3)] hover:shadow-[0_0_25px_rgba(255,64,129,0.7)] animate-pulse-glow transition-all duration-300 border border-[#2d2418]/10 hover:border-bloom-rose/60 active:scale-95 min-h-[44px] min-w-[44px] flex items-center justify-center"
      ...
      <div className="relative w-full max-w-5xl my-auto max-h-[90vh] glass-strong rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl shadow-[0_0_60px_rgba(255,64,129,0.25)] border border-[#2d2418]/10 flex flex-col lg:flex-row bg-[#FFF8F3]/90 animate-scale-in">
        {/* Left Side: Image Display */}
        <div className="relative lg:w-3/5 bg-gradient-to-br from-bloom-blush/60 via-bloom-rose/10 to-bloom-lavender/40 flex items-center justify-center p-4 sm:p-8 min-h-[260px] sm:min-h-[420px] overflow-hidden group shrink-0">
          ...
          <div className="absolute top-3 left-3 sm:top-4 sm:left-4 p-1.5 sm:p-2 rounded-xl sm:rounded-2xl glass text-emerald-400 flex items-center gap-1.5 text-[10px] sm:text-xs font-medium border border-emerald-500/20 z-10">
          ...
          <div
            ref={thumbRef}
            className="absolute bottom-2 sm:bottom-3 left-1/2 -translate-x-1/2 max-w-[90%] flex gap-1.5 sm:gap-2 overflow-x-auto p-1 sm:p-1.5 rounded-xl sm:rounded-2xl glass border border-[#2d2418]/10 scrollbar-none scroll-smooth"
          >
        ...
        <div className="lg:w-2/5 p-4 sm:p-8 flex flex-col justify-between overflow-y-auto max-h-[50vh] lg:max-h-[90vh]">
          <div>
            ...
            {/* Title */}
            <h2 className="text-xl sm:text-3xl font-serif font-bold text-[#2d2418] mb-2 leading-tight break-words">
              {currentItem.name}
            </h2>

            {/* Rating & Price */}
            <div className="flex items-center justify-between my-2 sm:my-3 pb-2 sm:pb-3 border-b border-[#2d2418]/10">
              <div className="flex items-center gap-1.5 text-bloom-gold text-xs sm:text-sm font-medium">
                ...
                <span className="text-[#8a7a6a] text-[10px] sm:text-xs font-normal">({currentItem.reviewCount} reviews)</span>
              </div>
              <div className="text-right">
                <span className="text-xl sm:text-2xl font-bold text-[#2d2418]">₹{currentItem.price}</span>
                {currentItem.originalPrice && (
                  <span className="text-xs sm:text-sm text-[#8a7a6a] line-through ml-2">₹{currentItem.originalPrice}</span>
                )}
              </div>
            </div>

            {/* Description */}
            <p className="text-[#6b5d4f] text-xs sm:text-sm leading-relaxed mb-4 sm:mb-6 font-sans line-clamp-4 sm:line-clamp-none">
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-4 sm:mb-6">
              {currentItem.tags.map((tag) => (
                <span key={tag} className="text-[10px] sm:text-xs px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg glass text-pink-300 font-mono">
                  #{tag}
                </span>
              ))}
            </div>
          ...
          {/* Footer Actions */}
          <div className="pt-3 sm:pt-4 border-t border-[#2d2418]/10 flex flex-col gap-2.5 sm:gap-3">
            <div className="flex gap-2 sm:gap-3">
              <button
                ...
                className={`flex-1 py-2.5 sm:py-3 px-3 sm:px-4 rounded-xl sm:rounded-2xl font-medium text-xs sm:text-sm flex items-center justify-center gap-2 transition-all border ${
                  isLiked
                    ? 'bg-rose-500/20 text-rose-700 border-rose-500/40 shadow-lg shadow-rose-500/20'
                    : 'glass text-[#6b5d4f] hover:text-[#2d2418] border-[#2d2418]/10 hover:border-bloom-rose/40'
                }`}
              >
              ...
              <button
                onClick={handleShare}
                className="py-2.5 sm:py-3 px-3 sm:px-4 rounded-xl sm:rounded-2xl glass text-[#6b5d4f] hover:text-[#2d2418] border border-[#2d2418]/10 hover:border-[#2d2418]/20 text-xs sm:text-sm font-medium flex items-center gap-2 transition-all relative shrink-0"
...
export default function Gallery() {
  useSEO({ title: "Bloomwire — Gallery | Customer Creations", description: "See real Bloomwire creations shared by our customers. Handcrafted pipe cleaner flowers in stunning arrangements.", canonicalPath: "/#/gallery" })
  ...
  const allItems: GalleryItem[] = products
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-8 sm:mb-10 reveal">
          ...
          <p className="text-[#8a7a6a] text-xs sm:text-base max-w-xl mx-auto leading-relaxed px-2">
        ...
        <div className="mb-8 sm:mb-12 space-y-4 sm:space-y-6 reveal">
          {/* View Toggle */}
          <div className="flex items-center justify-between gap-3 sm:gap-4 glass p-2.5 sm:p-3 rounded-2xl border border-[#2d2418]/10 max-w-4xl mx-auto shadow-lg">
            <div className="flex items-center gap-1.5 bg-white/80 p-1 rounded-xl border border-bloom-rose/15 shadow-sm">
              <button
            ...
            <div className="text-[11px] sm:text-xs text-[#8a7a6a] font-mono px-2 truncate">
            ...
            <button
              ...
              className={`shrink-0 whitespace-nowrap min-h-[40px] px-4 sm:px-5 py-2 sm:py-2.5 rounded-full text-xs sm:text-sm font-medium flex items-center justify-center transition-all duration-300 active:scale-95 ${
                !activeCat
                  ...
                  : 'glass text-[#6b5d4f] hover:text-[#2d2418] hover:border-bloom-rose/30 border border-[#2d2418]/10'
              }`}
            >
        ...
        <div className={`transition-all duration-300 ease-out ${isFiltering ? 'opacity-0 scale-[0.98]' : 'opacity-100 scale-100'}`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 relative z-10">
                {filteredItems.map((item, i) => {
                  ...
                  return (
                    ...
                    >
                      <div className="relative p-1 rounded-[32px] sm:rounded-[40px] rounded-tl-sm border border-[#2d2418]/10 group-hover:border-bloom-rose/50 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] shadow-xl group-hover:shadow-[0_0_30px_rgba(255,64,129,0.35)] group-hover:scale-[1.03] overflow-hidden">
                        ...
                        {/* Leaf Card Interior */}
                        <div className="relative z-10 rounded-[30px] sm:rounded-[38px] rounded-tl-sm overflow-hidden bg-[#FFF8F3]/90 backdrop-blur-md">
                          ...
                          <button
                            ...
                            className={`absolute top-3 right-3 sm:top-4 sm:right-4 z-20 p-2.5 sm:p-3 rounded-full glass border transition-all ${
                              userLikedThis
                                ? 'border-rose-500 text-rose-400 bg-rose-500/20 shadow-lg'
                                : 'border-[#2d2418]/10 text-[#6b5d4f] hover:text-[#2d2418] hover:border-bloom-rose/50'
                            }`}
                          ...
                          <div className="relative aspect-[4/5] min-h-[220px] sm:min-h-[280px] overflow-hidden bg-gradient-to-b from-bloom-blush/40 to-bloom-rose/10">
                            <img
                              src={item.src}
                              alt={item.name}
                            ...
                            <div className="absolute inset-0 bg-gradient-to-t from-bloom-darker/95 via-bloom-darker/60 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-4 sm:p-6">
                              <div className="transform translate-y-3 group-hover:translate-y-0 transition-transform duration-300">
                                ...
                                <div className="flex items-center justify-between text-xs text-[#6b5d4f] pt-2 border-t border-[#2d2418]/10">
                          ...
                          <div className="p-3 sm:p-4 bg-[#FFF8F3]/80 flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-t border-[#2d2418]/10">
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="text-[11px] text-emerald-400/80 flex items-center gap-1 truncate">
                                <LeafIcon className="w-3 h-3 shrink-0" /> <span className="truncate">Handcrafted in Jaipur</span>
                              </p>
                            </div>
                            <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center border-t sm:border-t-0 pt-2 sm:pt-0 border-[#2d2418]/10 shrink-0">
                              <span className="text-xs sm:text-sm font-bold text-bloom-gold">₹{item.price}</span>
                              {item.originalPrice && (
                                <span className="text-[10px] text-[#8a7a6a] line-through sm:ml-0 ml-1.5">₹{item.originalPrice}</span>
                              )}
                            </div>
                          </div>
                        </div>
          ...
          {viewMode === 'masonry' && (
            <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-4 sm:gap-6 py-4 sm:py-6">
              {filteredItems.map((item, i) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => setLightboxIndex(filteredItems.indexOf(item))}
                  >
                    <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden glass-strong border border-[#2d2418]/10 group-hover:border-bloom-rose/50 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.03] shadow-xl group-hover:shadow-[0_0_30px_rgba(255,64,129,0.35)]">
                      <div className={`relative ${aspectClass} min-h-[220px] sm:min-h-[260px] overflow-hidden bg-[#FFF8F3]`}>
                        <img
                          src={item.src}
                          alt={item.name}
                          className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out"
                        ...
                        <div className="absolute top-2.5 left-2.5 right-2.5 sm:top-3 sm:left-3 sm:right-3 flex items-center justify-between z-10 pointer-events-none">
                          <span className="px-2.5 py-1 rounded-full text-[10px] font-medium uppercase tracking-wider bg-white/80 backdrop-blur-sm text-[#2d2418] border border-bloom-rose/15 shadow-sm truncate max-w-[70%]">
                            {item.category}
                          </span>
                          <button
                            onClick={(e) => toggleLike(item.id, e)}
                            className={`pointer-events-auto p-2 rounded-full glass transition-all ${
                              userLikedThis
                                ? 'text-rose-400 fill-rose-400 bg-rose-500/20 border-rose-500/50'
                                : 'text-[#2d2418]/80 hover:text-[#2d2418] border-[#2d2418]/10'
                            }`}
                            title="Like this bloom"
                          >
